<!-- 20190616 - Added a table to display the data read from the form. Added an inline button to clear the form - Banele -->
<!-- 20190617 - Added a footer. Fixed the table display as it was not displaying other form fields data - Banele -->
<!-- 20190618 - Added a checkbox called Inactive - Banele -->
<!-- 20190619 - Fixed the prepared insert sql statement as it was not working because I was using the backticks signs instead of single quotes in the column names. - Banele -->
...
<?php
  ...
  if(isset($_POST["submit"])){
    ...
    // $stmt = $conn->prepare("INSERT INTO `user` (`usr_firstname`, `usr_lastname`, `usr_IdNumber`, `usr_dob`, `usr_gender`, `usr_userType`, `usr_userDateCreated`, `usr_status`, `usr_userNote`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    // $stmt->bind_param("sssssssss", $fname, $lname, $id, $dob, $dateCreated, $usrType, $gndr, $usrStatus, $usrNote);
    // $stmt->execute();
  }
?>
